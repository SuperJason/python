...
/*  __builtin_bswap32 is gcc build-in function */
...
void sha256(const unsigned char *data, size_t len, unsigned char *out)
{
	...
	size_t new_len = len + append + 8; /* Here 8 is 64bits */
	unsigned char buf[new_len];

	bzero(buf + len, append); /* bzero is from <string.h> */
	if (len > 0) {
		memcpy(buf, data, len);
	}
	...
	for (int idx = 0; idx < chunk_len; idx++) {
		...
		for (int i = 0; i < 64; i++) {
			uint32_t s_1 = ror32(e, 6) ^ ror32(e, 11) ^ ror32(e, 25);
			uint32_t ch = (e & f) ^ (~e & g);
			uint32_t temp1 = h + s_1 + ch + k[i] + w[i];
			uint32_t s_0 = ror32(a, 2) ^ ror32(a, 13) ^ ror32(a, 22);
